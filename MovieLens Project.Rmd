...
```{r}
#Since we don't want to download and process the dataset every time we run this report as it takes a significant time, we run it once in the movielens R script, and export the environment to an RData file, that already contains the edx, validation and edx_year_sanitized. 
...
# If from some reason we want to regenerate the RData, we can comment this line out, and enable the following code chunks
...
# If from some reason we want to regenerate the RData again, we should uncomment these lines. 
