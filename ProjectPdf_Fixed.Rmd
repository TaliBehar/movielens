...
Recommendation systems are the systems that are designed to recommend things to the user based on many different factors.

The goal of a recommender system is to generate meaningful recommendations, by predict the "rating" or "preference" a user would give to an item, to a collection of users for items or products that might interest them. 
...
2.	The characteristics we will use for the prediction:
  ...
  * **User**: users are different with their personal preferences, as well as in them engagement and excitement level. We’ll check the user specific effect on the mean.
  ...
  * **Movie age**: older movies tend to be rated higher than new movies. We’ll check how the age of the movie effect on the user ratings. 
...
**Method**
  ...
  * Penalize outliers - cross validation in order to find penalty term- diving the data once againin order to prevent overfiting
...
The chosen model was test on the validation set and resulted in X <FILL>
<TABLE>


**Conclusion**

The chosen model is the one that adds the genres and time effect to the average rating for a movie i and the specific effect of user u.
...
$$ MSE = {\frac{1}{N}\displaystyle\sum_{u,i} (\hat{y}_{u,i}-y_{u,i})^{2}} $$
...
outliers can be interuped ab b's "mistakes" -

  * Users (with more than average ratings) that systematically rates only 1 stars or only 5 stars 
...
Therefore it is much right to medel them together. 
...
in this case, if a cranky user (= negative $b_{u,i}$) rates a great movie (= positive $b_{i}$ ), the effects counter each other and we may be able to correctly predict that this user gave this great movie a 3 rather than a 5, and that should improve our prediction.
