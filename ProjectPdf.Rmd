...
```{r if required packeges, include=FALSE}
...
However, we only gets about 9 million rated movies insted of 746 million. that's because each user 
rated selected movies by his personal preference. By building well fitted model, we could fill-in 
these "holes" ....
```{r Overview summarize, echo=FALSE}
...
The following matrix contain random sample of 160 movies and 160 users...
```{r Movie Rating Combination Matrix, echo=FALSE, fig.align="center", message=FALSE, warning=FALSE}
# figure 1 # 
...
edx data set contains a timestamp column. 
This date-time is a point on the timeline, stored as the number of seconds since 1970-01-01 00:00:00 UTC. 
```{r sanitizing the data, echo=TRUE, message=FALSE, warning=FALSE}
# convert timestamp into rate date, add rate year column
...
glimpse(edx_year_sanitized, width = 60)
...
the most frequent value of ratings is only 20 movies, but there are 4.2 percent of users who rated more than 500 movies! 
...
```{r userId summarize, echo=FALSE, message=FALSE, warning=FALSE} 
...
```{r userId plots, echo=FALSE, message=FALSE, warning=FALSE}
# figure 2 # 
...
```{r movieId summarize, echo=FALSE, message=FALSE, warning=FALSE}   
...
```{r movieId plots, echo=FALSE, message=FALSE, warning=FALSE}
# figure 3 # 
...
```{r rating score summarize, echo=FALSE, message=FALSE, warning=FALSE}  
...
```{r rating score plots, echo=FALSE, message=FALSE, warning=FALSE}
# figure 4 # 
...
```{r Top 15 blockbuster movies, echo=FALSE, message=FALSE, warning=FALSE}
# figure 5 # 
...
```{r time frame ranges, echo=FALSE, message=FALSE, warning=FALSE}  
...
```{r year and rating, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r year and score, echo=FALSE, message=FALSE, warning=FALSE}
# figure 7 # (year + score) # figure 7 leads to age effect
...
```{r year rate score, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r top 15 movies, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r rate year plots, echo=FALSE, message=FALSE, warning=FALSE}
# figure 9 # 
...
```{r age plots, echo=FALSE, message=FALSE, warning=FALSE}
# figure 11 # 
...
```{r release year example, echo=FALSE, message=FALSE, warning=FALSE}
age_ex._1984 <- 
  edx_year_sanitized %>% 
  group_by(title, rate_year) %>%
  summarize(count=n(),
            ...
            rate_year=rate_year[1],
            release_year=release_year[1]) %>% 
  filter(release_year=="1984" & count>1000) %>% # filter movies with less than 1000 ratings
  group_by(rate_year) %>%
...
```{r release year example plot, echo=FALSE, message=FALSE, warning=FALSE}
# figure 12 # 
# 1984 release year example
...
```{r age of movie at rating, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r age of movie at rating summarize, echo=FALSE, message=FALSE, warning=FALSE}  
edx_year_age_sanitized %>% 
...
```{r age of the movie plots, echo=FALSE, message=FALSE, warning=FALSE}
# figure 13 #
...
ratings_by_age<- 
  ...
  geom_vline(aes(xintercept=mean(edx_year_age_sanitized$age_at_rating)),
...
score_by_age<- 
  ...
  geom_line(aes(color = k_count))+
  scale_color_gradient2(low = 'white', mid ='blue' , high = 'red')+
  geom_hline(aes(yintercept=mean(edx_year_age_sanitized$rating)),
                 color="blue", linetype="dashed", size=0.5)+
...
```{r age effect, echo=FALSE, message=FALSE, warning=FALSE}
...
style of filmmaking which became characteristic of American cinema between the 1910s (rapidly after World War I) and the 1960s

(https://en.wikipedia.org/wiki/Classical_Hollywood_cinema)

```{r classical age, echo=TRUE, message=FALSE, warning=FALSE}
classical_age <- 
  edx_year_age_sanitized %>% 
...
```{r classical age summarize, echo=FALSE, message=FALSE, warning=FALSE}  
classical_age %>% 
...
```{r old movies plot, echo=FALSE, message=FALSE, warning=FALSE}
# figure 16 #
...
```{r unique genres, echo=TRUE, message=FALSE, warning=FALSE}
# edx_genres <- edx_year_sanitized %>% separate_rows(genres, sep="\\|")

load("D:/OneDrive/Documents/Tali/Tali data science studies/movielens_dataset/movielens_dataset_all.RData")
```

```{r genres dist plot, echo=FALSE, message=FALSE, warning=FALSE}
# figure 17 #
...
```{r filter genres summarize, echo=FALSE, message=FALSE, warning=FALSE} 
...
```{r geners over the years plot, echo=FALSE, message=FALSE, warning=FALSE}
# figure 18 #
...
# if using R 3.5 or earlier, use `set.seed(755)
...
the goal is to aspire for RMSE as low as 0.864 

creating results table with naive approach
```{r naive rmse results}
...
```{r user rating ave, echo=FALSE, message=FALSE, warning=FALSE}  
user_rating_score <-
  train_edx %>% 
  group_by(userId) %>% 
...
```{r model 1 RMSE}
model_1_rmse <- 
  RMSE(true_ratings=test_edx$rating,
...
```{r modeling user effect - check + visualization, message=FALSE, warning=FALSE, include=FALSE}
...
```{r biggest "mistakes", echo=FALSE, message=FALSE, warning=FALSE}
check_predicted_u %>% 
...
```{r user_ave ratings vs. predicted, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r graph the residuals and se, message=FALSE, warning=FALSE}
...
using cross-validation to choose lambda, which is tuning parameter
```{r cv}
...
We know from experience that some movies are just generally rated higher than others. This intuition, that different movies are rated differently, is confirmed by data

b_i - average rating of movie i regardless of user
```{r fit movie ave}
fit_movie_ave <- 
  train_edx %>% 
  group_by(movieId) %>% 
...
```{r movie ave plots, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r modeling movie effect - check + visualization, message=FALSE, warning=FALSE, include=FALSE}
...
```{r movie ave ratings vs. predicted, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r graph the movie residuals and se, message=FALSE, warning=FALSE}
...
```{r regularized estimates vs least squre estimates }
...
```{r check predicted m reg, message=FALSE, warning=FALSE, include=FALSE}
...
```{r best movies regularized, echo=FALSE, message=FALSE, warning=FALSE}
best_movies_reg <- 
  fit_reg_movie_ave %>%
  left_join(titles, by="movieId") %>%
  group_by(title)%>%
...
```{r modeling movie user effect - check + visualization, message=FALSE, warning=FALSE, include=FALSE}
...
```{r graph the movie user residuals and se, message=FALSE, warning=FALSE}
...
```{r lambda for movie user effect, echo=TRUE, message=FALSE, warning=FALSE}
...
apply lambda on edx train+test
```{r apply lambda movie user, message=FALSE, warning=FALSE}
...
#### forth model - modeling regularized user+age_at_rating effect 
...
# choosing penalty term (lambda) for user age effect
```{r lambda for user age effect, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r penalty term user age, echo=TRUE, message=FALSE, warning=FALSE}
...
apply lambda on edx train+test
```{r apply lambda user age, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r predict user age reg ave, echo=TRUE, message=FALSE, warning=FALSE}
predicted_ratings <- 
  test_edx %>%
  left_join(fit_reg_user_ave, by='userId') %>%
  left_join(fit_reg_user_age_ave, by='age_at_rating') %>%
...
```{r add time effect to reg movie user, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r predict time movie user effect, echo=TRUE, message=FALSE, warning=FALSE}
...
separate genres rows

```{r separate genres rows, echo=TRUE, message=FALSE, warning=FALSE}

train_edx_genre <- train_edx %>%  separate_rows(genres, sep="\\|")
test_edx_genre <- test_edx %>%  separate_rows(genres, sep="\\|")

```
#### sixth model - add Genre effect to age+user, movie+user 

```{r add age effect to reg movie user, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r predict age effect to reg movie user, echo=TRUE, message=FALSE, warning=FALSE}
...
model_6_rmse <- RMSE(true_ratings=test_edx_genre$rating,
                     predicted_ratings=predicted_ratings)
rmse_results <- bind_rows(rmse_results,
                data_frame(method="add Genre effect to age+ reg. movie + user ",                            RMSE = model_6_rmse ))
rmse_results %>% knitr::kable()
```

#### seventh model - add Genre effect to age+user, movie+user
```{r movie user time genres, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r predict movie user time genres, echo=TRUE, message=FALSE, warning=FALSE}
...
rmse_results <- bind_rows(rmse_results,
                data_frame(method="add Genre effect to time+ reg. movie + user ",                            RMSE = model_7_rmse ))
rmse_results %>% knitr::kable()
```

#### model_7_rmse 0.8640 perfect!
